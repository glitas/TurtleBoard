...
define(['jquery', 'jqueryui', 'github', 'vis', 'underscore', 'N3', 'splitPane',
		...
		'mode/turtle/turtle', 'hint/turtle-hint', 'logger', 'rdfstore'],

function ($, JQueryUI, Github, vis, underscore, N3, SplitPane, CodeMirror, ShowHint, SearchCursor,
	MatchesOnScrollbar, AnnotateScrollbar, ModeTurtle, HintTurtle, logger, rdfstore) {
  ...
  // HTML elements ------------------------------------------------------------
	...
	// :turtle:Board - Clear Editor Function ----------------------------------------------
	$("#tabs").on("click", "#clear-button", function () {
		$(document).ready(function() {
			$.ajaxSetup({ cache: false });
		});
		editor.setValue("");
		jQuery.get('prefixes.txt',function(txt){
			editor.setValue(txt)
		});
	});
	//-------------------------------------------------------------------------------------
	// :turtle:Board - Get Final Solution Function ----------------------------------------
	$("#tabs").on("click", "#solution-button", function () {
		$(document).ready(function() {
			$.ajaxSetup({ cache: false });
		});
		editor.setValue("");
		jQuery.get('vocab/' + tn +'_sol.ttl',function(ttl){
			editor.setValue(ttl)
		});	
	});
	//------------------------------------------------------------------------------------
	// :turtle:Board - Get Guidelines and Get Hints Function / Parsering------------------
	
	var h = 'ontologies/' + tn + '.ttl';
	//console.log(h);
	
	//arrays for storing the parsed data
	var gset = [];   // guidelines
	var eset = [];   // examples
	var tset = [];   // tasks
	var hset = [];   // hints
	var sset = [];   // solution triples for ASK query
	var clean = [];  // cleaned solution triples for ASK query
	var solset = []; // solution triples from solution file
	...
	jQuery.get(h, function(data){
		var parser = N3.Parser();
		parser.parse(data,
             function (error, triple, prefixes) {
				   // console.log(data);
				   // console.log(triple.subject, triple.predicate, triple.object, '.');
				   // checking the triples
					if (triple.predicate === "http://example.org/#exampleText")
					{ 
						//console.log(triple.predicate);
						gset[glen] = triple.object;
						glen = glen + 1;
					}
					
					else if (triple.predicate === "http://example.org/#exampleAnswer")
					{
						eset[elen] = triple.object;
						elen = elen + 1;
					}
					
					else if (triple.predicate === "http://example.org/#question")
					{
						tset[tlen] = triple.object;
						tlen = tlen + 1;
					}
					
					else if (triple.predicate === "http://example.org/#sparqlAskValidation")
					{
						sset[slen] = triple.object.replace(/\"/g, "");
						//slen = slen + 1;
						
						
						var string = sset[slen];
						string = string.split(" ");
						...
						for (var k = 0; k < stringArray.length; k++) {
							stringArray[k] = stringArray[k].replace(/\./g, "");
						} 
    
    
						for (var j = 0; j < stringArray.length; j++) {
							//document.getElementById(j).innerHTML = stringArray[j];
							clean.push(stringArray[j]);
					...
					else if (triple.predicate === "http://example.org/#hint")
					{
						hset[hlen] = triple.object;
						hlen = hlen + 1;
	...
	// getting the prefixes file
	var filePath = "prefixes.txt";
	xmlhttp = new XMLHttpRequest();
	xmlhttp.open("GET",filePath,false);
	xmlhttp.send(null);
	
	// cleaning the prefixes file
	var prefixesFile = xmlhttp.responseText.replace(/\@/g, "");	
	prefixesFile = prefixesFile.replace(/\> ./g, ">");
	prefixesFile = prefixesFile.replace(/\>./g, ">");
	prefixesFile = prefixesFile.replace(/\prefix/g, "PREFIX");

	//console.log(prefixesFile);

	// getting the solution file
	var filePath1 = 'vocab/' + tn +'_sol.ttl';
	xmlhttp1 = new XMLHttpRequest();
	xmlhttp1.open("GET",filePath1,false);
	xmlhttp1.send(null);
	var solFile = xmlhttp1.responseText;	
	
	//console.log(solFile);
	
	var s = 'vocab/' + tn +'_sol.ttl';
	
	// parsing of the solution file
	jQuery.get(s, function(data){
		...
		parser.parse(data,
             function (error, triple, prefixes) {
					if (triple)
					{ 
						solset[sollen] = triple.subject;
						solset[sollen+1] = triple.predicate;
						solset[sollen+2] = triple.object;
						sollen = sollen + 3;
					}
					 else
						console.log("");
	...
	// :turtle:Board - Printing the guidelines, examples and tasks ------------------------------------
		
	var timet = 0;
	$("#tabs").on("click","#next-button",function() {
		if (timet === 0)
		{
			timer();
			timet = timet + 1;
		...
		if (gset[gi]) {
			document.getElementById('guide').innerHTML = "<br/><br/><font size='4'><strong>GUIDELINE: " + (gset[gi].replace(/\"/g, "")) + "</strong></font><br/>";
			document.getElementById('ex').innerHTML = "<font size='4' color='#2ECC71'><strong><u>EXAMPLE: " + (eset[gi].replace(/\"/g, "")) + "</u></strong></font><br/><br/>";
			document.getElementById('task').innerHTML = "<br/><font size='5'><strong>TASK: " + (tset[gi].replace(/\"/g, "")) + "</strong></font><br/><br/>";
			document.getElementById('steps').innerHTML = "<br/><font size='4.8'><strong>Step "+ (gi+1) + " / " + (gset.length) +"</strong></font><br/>";
			gi = gi + 1;
		}
		else {
			document.getElementById('guide').innerHTML = "<br/><br/><font size='4'><strong>" + "You reached the end of this tutorial." + "</strong></font><br/>";
			document.getElementById('ex').innerHTML = "<font size='4'><strong>" + "-----------------------------------------------" + "</strong></font><br/><br/>";
			document.getElementById('task').innerHTML = "<br/><font size='4.8'><strong>" + "THE END" + "</strong></font><br/><br/>";
			timer_out();
			//document.getElementById('steps').innerHTML = "<br/><font size='4'><strong></strong></font><br/>";
		}
	});
	
	
	// Board - Printing the hints ------------------------------------

	$("#tabs").on("click","#hint-button",function() { 
		if (hset[hi]) {
			document.getElementById('hint').innerHTML = "<br/><font size='4'><strong>" + (hset[hi].replace(/\"/g, "")) + "</strong></font><br/>";
			document.getElementById('hscore').innerHTML = hi+1;
			hi = hi + 1;
		}
		else {
			document.getElementById('hint').innerHTML = "<br/><font size='4'><strong>" + "You have used all hints!" + "</strong></font><br/>";
			document.getElementById('hico').style.visibility = "visible";
		}
	...
	// Board - Comparison (Check Your Solution) Function ------------------------------------
	var clicks = 0;
	var h1 = 'prefixes.txt';
	
	$("#tabs").on("click","#check-button", function() { 
	
	//document.getElementById('answer').innerHTML = "Clicked"; 
	
	var results;
	var vocabulary = editor.getValue().toString('utf-8');
	...
	var varia1 = editor.getLine(lastLine);
	var varia = editor.getLine(lastLine).replace(/\./g, "");	
	//console.log(varia);
	
	//SOLUTIONS
	//testing vars
	//var subject1 = 'http://www.example.com#Garfield';				        //the value of the subject from the solution
	//var predicate1 = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type';	//the value of the predicate from the solution
	//var object1 = 'http://www.example.com#cat';							//the value of the object from the solution
	
	//the actual triple variables from the solution files
	var subject1 = clean[clicks];
	var predicate1 = clean[clicks + 1];
	var object1 = clean[clicks + 2];
	
	
	var subject2 = solset[clicks];
	var predicate2 = solset[clicks + 1];
	var object2 = solset[clicks + 2];
	...
	//console.log(predicate1);
	//console.log(object1);
	
	//console.log(subject2);
	//console.log(predicate2);
	//console.log(object2);
	
	// ASK query
	queryString = prefixesFile + "ASK  {" + varia + "}";
	
	
	// checking the triples	
		rdfstore.create(function(err, store) 
		{
		  store.load('text/turtle', solFile, function(err, results)
		  {
			if(err)
			{
				document.getElementById('answer').innerHTML = "Could not run SPARQL Query!"; 
			}
			else
			{
				// runing the query
				...
				{
					document.getElementById('answer').innerHTML = "<font size='5'><strong>" + results + "</strong></font>"; 
					//console.log(results);
					
					jQuery.get(h1, function(data){
					var parser1 = N3.Parser();
					parser1.parse(data + '\n' + '' + varia1 + '', 
				...
				function (error, triple, prefixes) {
				  ...
				  // console.log(subject1);
					...
					{
						//console.log(subj);
						document.getElementById('sanswer').innerHTML = "<font size='3' color='#2ECC71'><strong><u>SUBJECT:</u> correct </strong></font>"; 
					}
				   else
					{	   
					   
					    nmist = nmist + 1;
					  	document.getElementById('sanswer').innerHTML = "<font size='3' color='red'><strong><u>SUBJECT:</u> wrong </strong></font>"; 
						document.getElementById('mistakes').innerHTML = nmist;
					...
					{
					   //console.log(pred);
					   document.getElementById('panswer').innerHTML = "<font size='3' color='#2ECC71'><strong><u>PREDICATE:</u> correct </strong></font>"; 
					...
					else
					{
						...
						nmist = nmist + 1;
						document.getElementById('panswer').innerHTML = "<font size='3' color='red'><strong><u>PREDICATE:</u> wrong </strong></font>"; 
						document.getElementById('mistakes').innerHTML = nmist;
					...
					{
						//console.log(obj);
						document.getElementById('oanswer').innerHTML = "<font size='3' color='#2ECC71'><strong><u>OBJECT:</u> correct </strong></font>"; 
					...
					else
					{
						...
						nmist = nmist + 1;
						document.getElementById('oanswer').innerHTML = "<font size='3' color='red'><strong><u>OBJECT:</u> wrong </strong></font>"; 
						document.getElementById('mistakes').innerHTML = nmist;
